...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_rate, use_stride=True):
        super().__init__()

        modules = [nn.Dropout(p=dropout_rate)] if dropout_rate > 0 else []
        modules.extend([
            (nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=2, padding=1) if use_stride
                else nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3)),
            nn.ReLU()
        ])
...
class ConvNet(nn.Module):
    def __init__(self, hidden_size, output_size):
        ...
        self.conv = nn.ModuleList([
            ConvLayer(in_channels=1, out_channels=16, dropout_rate=0, use_stride=False),
        ...
        self.linear = nn.ModuleList([
            LinearLayer(in_channels=(26 // 2 // 2 + 1) ** 2 * 64, out_channels=hidden_size, dropout_rate=dropout_rate, relu=True),
